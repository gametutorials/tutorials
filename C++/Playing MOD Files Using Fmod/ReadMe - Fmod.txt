...
Files:  	Main.cpp (The source file containing the actual code)
		Fmod.cpp (The source file containing the "FMod class implementation")
		CFmod.h (The header file containing the "Fmod class definition")
		Fmod.h (The header file containing © FireLight Multimedia's Fmod definitions)
		Fmod.vcxproj  (The VC++ project file holding the project info)
		Fmod.sln  (The solution file holding the solution info)
		techno.mod (The MOD file which holds the song data)

Libraries:	fmod.dll   (The Dynamic Link Library needed in order to run the program - © FireLight Multimedia)
		fmodvc.lib (The Fmod Library needed to compile the program - © FireLight Multimedia)
